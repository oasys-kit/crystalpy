...
if __name__ == "__main__":
    ...
    xtal_size = numpy.array([200,400,500,600,700,800,900,1000,2000,5000,10000])
    wavelength = 1.540598e-10
    photon_energy = codata.h * codata.c / codata.e / wavelength
    print("photon_energy is: ", photon_energy)

    RA001 = numpy.zeros_like(xtal_size, dtype=float)
    RA111 = numpy.zeros_like(xtal_size, dtype=float)
    for i in range(xtal_size.size):
        try:
            ba, fwhm = compute_crystalpy_profile(do_plot=0,miller_h=0, miller_k=0, miller_l=1,
            ...
            RA001[i] = numpy.degrees(scherrer_fwhm) / fwhm
        except:
            pass

    for i in range(xtal_size.size):
        try:
            ba, fwhm = compute_crystalpy_profile(do_plot=0,miller_h=1, miller_k=1, miller_l=1,
                                                 thickness=xtal_size[i]*1e-9)
            theta_bragg = numpy.radians(ba / 2)
            D = xtal_size[i]*1e-9
            scherrer_fwhm = 4 * numpy.sqrt(numpy.log(2) / numpy.pi) * wavelength * numpy.abs(
                numpy.sin(theta_bragg)) / D / numpy.sin(2 * theta_bragg)
            print('scherrer, fwhm, ratio: ', numpy.degrees(scherrer_fwhm), fwhm, numpy.degrees(scherrer_fwhm) / fwhm)
            RA111[i] = numpy.degrees(scherrer_fwhm) / fwhm
        except:
            pass


    from srxraylib.plot.gol import plot
    plot(xtal_size, RA001,
         xtal_size, RA111,
         xlog=1, marker=['+','x'], legend=['Tripalmitin 001', 'Tripalmitin 111'], yrange=[0,1.2], xrange=[10,10000],

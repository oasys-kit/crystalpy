...
EXAMPLES = [
    ("Si111_simple.py",
     "Si 111 simple angular scan"),
    ("Si111_energy_scan.py",
     "Si 111 energy scan and double-crystal monochromator"),
    ("Si111_simple_xraylib_dabax_comparison.py",
     "Si 111: xraylib vs dabax comparison"),
    ("check_structure_factor.py",
     "Structure factor across backends"),
    ("check_scans_xcrystal.py",
...
# - sets the inline backend so plots appear in the notebook
# - patches srxraylib's set_qt() to a no-op (avoids Qt import in headless env)
SETUP_CELL = """\
%matplotlib inline
import matplotlib
...
def py_to_executed_notebook(py_path, ipynb_path, title, timeout=300):
    ...
    # run with cwd = examples/ so relative file paths in scripts still work
